...
public class SchedulesServiceImpl implements SchedulesService {
		...
		if (route.getAirports().size() == 2) {
			...
			for (FlightScheduleDto flightDtoFirstLeg : flightsScheduledFirstLeg) {
				...
				flightAuxDtoMinTimeStop
						.setDepartureDateTime(flightDtoFirstLeg.getDepartureDateTime().plus(minTimeStop));

				for (FlightScheduleDto flightDtoSecondLeg : flightsScheduledSecondLeg.tailSet(flightAuxDtoMinTimeStop,
						true)) {
					// create the flights
					Flight flight = new Flight();
					flight.setStops(1);
	...
	private NavigableSet<FlightScheduleDto> filterFlightsByDates(NavigableSet<FlightScheduleDto> totalFlights,
		...
		NavigableSet<FlightScheduleDto> flightsDepartureOnTime = totalFlights.tailSet(departureAuxDto, true);
		...
		NavigableSet<FlightScheduleDto> flightsArrivalOnTime = totalFlightsComparatorByArrival.headSet(arrivalAuxDto,
				true);
